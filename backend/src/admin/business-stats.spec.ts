...
describe('Business revenue', () => {
  ...
  it('queries only paid orders within the period and groups by Vietnam payment date', async () => {
    ...
    const aggregate = jest.fn().mockResolvedValue({ _sum: { amountVND: 900000 }, _count: 5 });
    const result = await new AdminService({ paymentOrder: { findMany, aggregate } } as unknown as PrismaService).getBusinessStats();
    ...
    expect(result.lifetimeRevenue).toBe(900000);
    expect(result.lifetimePaidOrders).toBe(5);
