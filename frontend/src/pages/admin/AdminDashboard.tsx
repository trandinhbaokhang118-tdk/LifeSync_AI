...
export function AdminDashboard() {
  ...
  const fetchDashboard = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (isRefresh) {
        showToast.success("Đã cập nhật", "Dashboard refreshed successfully");
      }
    } catch {
  ...
  const handleExportData = () => {
    const dataStr = JSON.stringify({ stats, logs }, null, 2);
    const dataBlob = new Blob([dataStr], { type: "application/json" });
    const url = URL.createObjectURL(dataBlob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `admin-dashboard-${new Date().toISOString()}.json`;
    link.click();
    URL.revokeObjectURL(url);
    showToast.success("Exported", "Dashboard data exported successfully");
  };
    ...
    <div className="admin-theme admin-container p-6 transition-colors duration-300 md:p-8">
      ...
      >
        <div>
          ...
          <p className="admin-title-sub">
            Live operational view · Last update:{" "}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <RefreshCw
              className={`h-4 w-4 ${autoRefresh ? "animate-spin-slow" : ""}`}
            />
          ...
          <Button
            ...
            disabled={refreshing}
            className="border-2 border-white/15 font-semibold text-white hover:border-cyan-400/40 hover:bg-cyan-500/10"
          >
            <RefreshCw
          ...
          <Button
            ...
            className="border-2 border-white/15 font-semibold text-white hover:border-green-400/40 hover:bg-green-500/10"
          >
